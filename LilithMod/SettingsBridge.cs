...
namespace LilithMod
{
    public sealed class SettingsBridge : MonoBehaviour
    {
        ...
        // Null forces a full restyle on the first tick after a (re)build; local
        // endpoints validate synchronously, so no false tick is guaranteed.
        private bool? _lastChatAvailability;
        ...
        private void BuildRows(TraySettingView view)
        {
            ...
            // Fresh field, fresh override styling on the next availability tick.
            _apiKeyOverridden = false;
            UpdateApiKeyFields();
            _pushToTalkKeyField.text = LilithModPlugin.CfgPushToTalkKey.Value ?? "F8";
        ...
        private void Sync()
        {
            ...
            // An overridden (non-interactable) field never writes back, so a saved
        ...
        {
            UpdateApiKeyField(_deepSeekKey, _deepSeekEye, _deepSeekRevealed, _apiKeyOverridden);
        }

        private static void UpdateApiKeyField(
            TMP_InputField field, Toggle eye, bool revealed, bool overridden)
        {
        ...
        /// <summary>Local endpoints ignore the key, so the field reads as inert.</summary>
        private void RefreshApiKeyOverride(bool overridden)
        {
            if (_apiKeyOverridden == overridden) return;
            _apiKeyOverridden = overridden;
            Color color = overridden ? DisabledColor : Color.white;
            if (_deepSeekKey != null)
            {
                _deepSeekKey.interactable = !overridden;
                if (_deepSeekKey.textComponent != null)
                    _deepSeekKey.textComponent.color = color;
            }
            if (_deepSeekLabel != null) _deepSeekLabel.color = color;
            UpdateApiKeyFields();
            // The base-URL flip changes what a valid key means; re-judge it.
            ScheduleApiKeyValidation(_deepSeekKey != null
                ? (_deepSeekKey.text?.Trim() ?? string.Empty) : string.Empty);
        }
        ...
        private void RefreshChatAvailability()
        {
            ...
            bool overridden = !LilithModPlugin.ApiKeyRequired;
        ...
        private static void OpenAiServiceConfig()
        {
            ...
            {
                LilithModPlugin.Logger.LogWarning("[Settings] Could not open the config: " + ex.Message);
            }
        }
