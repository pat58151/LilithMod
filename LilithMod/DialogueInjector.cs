...
{
    /// <summary>
    /// Loads custom dialogue from the plugin's custom/ folder into the game's live
    /// databases. Runs once, as soon as DialogueManager exists, then removes itself.
    /// </summary>
    public class DialogueInjector : MonoBehaviour
    {
        ...
        public DialogueInjector(System.IntPtr ptr) : base(ptr) { }
        ...
        public void Update()
        {
            ...
            if (_elapsed < _initialDelay) return;

            if (DialogueManager.s_instance != null)
            {
                // Authoring aid, off by default. It writes the game's own dialogue
                // to disk, which no player has a reason to do.
                if (LilithModPlugin.CfgDumpDialogueDatabase != null &&
                    LilithModPlugin.CfgDumpDialogueDatabase.Value)
                    DatabaseDumper.Run();
            ...
            if (_elapsed >= _timeout)
            {
                LilithModPlugin.Logger.LogWarning(
                    "[LilithMod] DialogueManager instance not found within timeout; custom dialogue not injected.");
                _done = true;
                Destroy(this);
            }
        }
