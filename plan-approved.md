...
## IMPLEMENTATION NOTE (added by reviewer - mandatory)
All DTOs MUST be plain managed C# classes holding plain managed types
(string, int, float, bool, System.Collections.Generic.List<T>).
Before serializing, explicitly convert every Il2Cpp collection to a managed one -
e.g. copy `Il2CppSystem.Collections.Generic.List<string> playerStates` element-by-element
into a `System.Collections.Generic.List<string>`. Newtonsoft.Json CANNOT serialize
Il2Cpp types (Il2CppSystem.*, Il2CppReferenceArray, Il2CppStringArray) and will throw or
emit garbage. This applies to playerStates, triggerTypes, nextIds, conditions, options,
and playerLineOptions. `dotnet build` will NOT catch a violation of this - it fails only at runtime.
