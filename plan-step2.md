...
1. **Expand a simplified authoring format** into the real game schema (DialogueNode, PlayerLineEntry, DialogueLineEntry, etc.).
...
#### 3.1 Safe base ranges
- **DialogueNode `.id`** and **`.lineId`** : ≥ 9 000 000  
- **PlayerLineEntry `.id`** and **`.groupId`** : ≥ 900 000  
- **PlayerLineEntry `.LineID`** : ≥ 9 000 000  

Before allocation, scan **all existing entries** in the live databases (`DialogueDatabase`, `DialogueLineDatabase`, `PlayerLineDatabase`) and collect the maximum used ID in each category. The actual starting ID is `max(existing_max, base - 1) + 1`. Keep separate counters for each ID category.

#### 3.2 Collision handling
- Maintain a **global set** of all assigned IDs (node id, lineId, playerLine id, playerLine LineID).  
...
2. For each simplified node entry:  
   a) **Allocate a node id** (`nodeId`) and a `lineId` (both use the node ID counter).  
   b) Create a `DialogueLineEntry` with:
      - `id` = `lineId`
      - `text` = the node’s `say` text  
      - `soundId` = empty string (no audio reference from custom content).  
   c) Create a `DialogueNode` instance and set all fields (see §5).  
   d) If `choices` are present and non‑empty → process each choice (see §4.1).  
   e) Store mapping `key → nodeId` for later `goto` resolution.  
   f) Add the constructed node, line entry, and any generated player‑line entries to injection lists.
...
2. **Create `PlayerLineEntry`:**
   - `id` = `plId`, `LineID` = `plLineId` (allocated), `groupId` = new unique group id (using player line counter).  
   - `text` = choice’s `text`.  
   - `playerStates` = empty list.  
   - `viewLimit` = 0 (unlimited).  
3. **Resolve the target node id (`nextNodeId`)**:  
   - If `reply` → generate a **new** reply node (with allocated id) containing the reply text, no choices, and no trigger.  
...
- It receives a new `nodeId` and a new `lineId`.  
...
- Its `lineId` is added to the `DialogueLineDatabase` via a new `DialogueLineEntry`.
...
**Constructed objects (DialogueNode, DialoguePlayerLineOption, PlayerLineEntry, DialogueLineEntry) are created using their parameterless constructors** (the game’s internal IL2CPP types).
...
**For raw passthrough nodes:** create them exactly as provided in JSON, deserializing into the same fields, with all lists properly cast to Il2Cpp types.
...
**Steps:**
1. **Add each generated `DialogueLineEntry`** to the live `DialogueLineDatabase` instance (found via `Resources.FindObjectsOfTypeAll`). If none exists, log a warning and skip line entry injection (node’s own text field will still be present).  
2. **Add each `DialogueNode`** to the chosen database’s `nodes` list (the list is an `Il2CppSystem.Collections.Generic.List<DialogueNode>`).  
3. **Add each `PlayerLineEntry`** to `DialogueManager.s_instance.GetPlayerLineDatabase().entries`.  
4. **Call `db.BuildIndex()`**, **`playerDb.BuildIndex()`**, and **`DialogueManager.s_instance.BuildIndex()`** to rebuild the aggregated index.  
5. **For each injected `DialogueNode`**, call `DialogueManager.s_instance.RegisterNodeWeight(node)`.  
...
4. **Self‑check**: `DialogueManager.s_instance.TryGetNode` returns `true` for at least one injected node ID. This is confirmed via the log.  
5. **No performance degradation**: injection happens once on startup; it does not run every frame and does not leak memory.
